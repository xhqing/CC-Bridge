# GLM-BRIDGE — Claude Code ↔ GLM-5.3 (z.ai) 适配器

CC-Bridge 框架的 GLM 上游适配器，对接 [z.ai](https://z.ai) 的 GLM-5.3（Coding Plan）。这是目前唯一已实现的上游适配器。

## 它做什么

- 把 Claude Code 发来的 Anthropic `/v1/messages` 请求体适配为 z.ai GLM-5.3 友好的形式（见下表）。
...
主要字段：`API_BASE`（z.ai 接口地址）、`API_KEY`（逗号分隔多个 z.ai KEY，支持容灾）、`MODEL_MAP`（`spoof->target` 映射对，支持多对，默认 `claude-opus-4-8->glm-5.3`）、`MODEL_THINKING`（按 target 模型配思考等级 `max`/`high`/`none`，默认全 `max`）。
